# Attribution

## Project

Privacy-first Android launcher designed for focus, speed, and simplicity.

Built entirely in native Kotlin, runs fully on-device, avoids tracking, and keeps distractions to a
minimum by emphasizing clarity and intentional interaction.

- Original author: [Pombo](https://github.com/pomboverso)
- Original repository: https://github.com/rama-io/okapi
- License: **GPL 3.0**
- License link: https://www.gnu.org/licenses/gpl-3.0

---

## Font Attribution

This project uses the **Jersey 25** font from the Soft Type Collection.

- Font name: **Jersey 25**
- Author: [Sarah Cadigan-Fried](https://github.com/scfried)
- Source: https://github.com/scfried/soft-type-jersey
- License: **OFL 1.1**
- License link: https://openfontlicense.org

The SIL Open Font License 1.1 is included in the project and applies to the font files.

---

## Icon Attribution

Icons used in this project:

- Icon set: **pixel-icon-library**
    - Author: [HackerNoon](https://github.com/hackernoon)
    - Source: https://github.com/hackernoon/pixel-icon-library
    - License: **CC 4.0**
    - License link: https://creativecommons.org/licenses/by/4.0/

---

[Go Back To Readme](../readme.md)